...
protected:
  BatchNormLayerTest()
    : blob_bottom_(new Blob<Dtype>(5, 2, 3, 4)), blob_top_(new Blob<Dtype>()) {
    // fill the values
...
#ifdef USE_CUDNN
template <typename TypeParam>
class FusedBatchNormLayerTest : public GPUDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

protected:
  FusedBatchNormLayerTest()
    : blob_bottom_(new Blob<Dtype>(5, 2, 3, 4)), blob_top_(new Blob<Dtype>()) {
    // fill the values
    FillerParameter filler_param;
    filler_param.set_mean(-100.);
  ...
  virtual ~FusedBatchNormLayerTest() {
    delete blob_bottom_;
    delete blob_top_;
  }
  void checkMeanVar(
      const Blob<Dtype> *blob, int num, int channels, int height, int width);
  Blob<Dtype>* const blob_bottom_;
  Blob<Dtype>* const blob_top_;
  vector<Blob<Dtype>*> blob_bottom_vec_;
  vector<Blob<Dtype>*> blob_top_vec_;
...
template <typename TypeParam>
void FusedBatchNormLayerTest<TypeParam>::checkMeanVar(
    const Blob<Dtype> *blob, int num, int channels, int height, int width) {
  for (int j = 0; j < channels; ++j) {
    float mean = 0, var = 0;
    for (int i = 0; i < num; ++i) {
      for (int k = 0; k < height; ++k) {
        for (int l = 0; l < width; ++l) {
          float data = blob->data_at(i, j, k, l);
          mean += data;
          var += data * data;
        }
      }
    ...
    const float kErrorBound = 0.001;
    EXPECT_NEAR(0, mean, kErrorBound);
    EXPECT_NEAR(1, var, kErrorBound);
  }
}

TYPED_TEST_CASE(FusedBatchNormLayerTest, TestDtypesAndDevices);

TYPED_TEST(FusedBatchNormLayerTest, TestForward) {
  ...
  FusedBatchNormLayer<Dtype> layer(layer_param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
...
TYPED_TEST(FusedBatchNormLayerTest, TestForwardInplace) {
  typedef typename TypeParam::Dtype Dtype;
  Blob<Dtype> blob_inplace(5, 2, 3, 4);
  vector<Blob<Dtype>*> blob_bottom_vec;
  vector<Blob<Dtype>*> blob_top_vec;
...
TYPED_TEST(FusedBatchNormLayerTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
